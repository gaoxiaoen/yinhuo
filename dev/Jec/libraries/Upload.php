...
class Upload
{
    ...
    private $limit_exts = array('php', 'php3', 'phtml', 'php4', 'asp', 'html', 'htm', 'js', 'vbs', 'bat','sh','bash');
    ...
    private function _upload($file_info)
    {
        ...
        $result['url']     = str_replace(WWW_PATH, '', $path['path']);
    ...
    private function _getFilename($fileInfo)
    {
        ...
        $new_name = $this -> rename ? time().rand(10000, 99999) : $name;
    ...
    private function _getFileSize($size)
    {
        ...
        $size_format = formatBytes($size);
        if (!$max_size)
            return $size_format;
        
        if ($size > $max_size)
        {
            $this -> error[] = "文件大小不能超过：" . formatBytes($max_size).'！';
            return false;
        }
