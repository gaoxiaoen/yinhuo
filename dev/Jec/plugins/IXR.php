...
EOD;
    ...
    function output($xml) {
        global $CONFIG;
        $xml = '<?xml version="1.0" encoding="'.$CONFIG['html_charset'].'" ?>'."\n".$xml;
        $length = strlen($xml);
        header('Connection: close');
        header('Content-Length: '.$length);
        header('Content-Type: text/xml; charset='.$CONFIG['html_charset']);
        header('Date: '.date('r'));
        echo $xml;
        exit;
    }
...
class IXR_Request {
    ...
    function IXR_Request($method, $args) {
        ...
        $this->args = $args;
        $this->xml = <<<EOD
<?xml version="1.0" encoding="{$CONFIG['html_charset']}" ?>
<methodCall>
...
class IXR_Client {
    ...
    function query() {
        ...
        $this->headers['Content-Type']    = 'text/xml; charset='.$CONFIG['html_charset'];
