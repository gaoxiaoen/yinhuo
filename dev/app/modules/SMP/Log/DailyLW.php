...
class SMP_Log_DailyLW extends AdminController
{

    public function __construct()
    {
        parent::__construct();
        $this->assign('title', '每日神兵日志');
    ...
    private function show(){
        ...
        foreach($data as $key => $val){
            ...
            $TermLv = $this->db_game->getOne("select after_lv from log_light_weapon_lv where pkey = {$pkey} and time <= {$nexttime}  order by time desc");
            if(empty($TermLv))$TermLv = 0;
            $DanRow = $this->db_game->getRow("select dan1_num,dan2_num,dan3_num from log_light_weapon_dan where pkey = {$pkey} and time <= {$nexttime} order by time desc");
            $Dan1Num = 0;
            $Dan2Num = 0;
            $Dan3Num = 0;
    ...
    private function csv_download($data)
    {
        ...
        $csv->addAll($data);
        $csv->download('log_daily_light_weapon.csv');
    }
