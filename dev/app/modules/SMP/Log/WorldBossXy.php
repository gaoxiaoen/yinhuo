 ...
 * Time: 0:17
 */
class SMP_Log_WorldBossXy extends AdminController
{

    public function __construct()
    {
        parent::__construct();
        $this->assign('title', '星蕴boss击杀日志');
        $this->show();
    }
    ...
    private function show(){
        ...
        $pager = new Pager();
        $pager->setTotalRows($this->db_game->getOne("select count(*) from log_world_boss_xy_kill where $time $where"));
        $offset = $pager->getOffset();
        $limit = $pager->getLimit();
        $data = $this->db_game->getAll("select * from log_world_boss_xy_kill  where $time $where order by time desc limit $offset,$limit");
