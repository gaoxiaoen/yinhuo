...
class SMP_Log_DfXunbao  extends AdminController
{

    public function __construct()
    {
        parent::__construct();
        $this->assign('title', '巅峰寻宝日志表');
    ...
    private function show(){
        ...
        $pager = new Pager();
        $pager->setTotalRows($this->db_game->getOne("select count(*) from log_df_xunbao where $time $where"));
        $offset = $pager->getOffset();
        $limit = $pager->getLimit();
        $data = $this->db_game->getAll("select * from log_df_xunbao where $time $where order by time desc limit $offset,$limit");
        global $Ggoods;
        foreach($data as &$d){
            $goodsstr = "";
        ...
        if (Jec::getVar('download')) $this->csv_download($this->db_game->getAll("select * from log_df_xunbao  where $time $where order by time "));
