...
class online_gift{
    ...
    public function get_erl_content($row){
        ...
        $title = $info[2];
        $content = $info[3];
        $rec = "get({$row['act_id']}) -> #base_online_gift{open_info={$openInfo},act_id={$row['act_id']},open_time={$opentime},gift_list={$giftlist},mail_title=?T(\"{$title}\"),mail_content=?T(\"{$content}\"),act_info={$actInfo} };\n";
        return $rec;
    }
