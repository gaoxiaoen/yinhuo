...
class SMP_Player_EquipPet extends AdminController
{
    public function __construct()
    {
        parent::__construct();
        $this->assign('title', '宠物装备查看');
        $act = Jec::getVar('act');
        if($act == 'update')
            $this->update();
    ...
    private function update(){
        ...
        Log::info('SMP_Player_EquipPet',"修改[{$gkey}][$file=$value]");
        $this->db_game->update('pet_goods',array($file=>$value),$where);
    }
    ...
    private function show(){
       ...
       $pos = Jec::getInt('kw_itempos');
        ...
        $pager = new Pager();
        $pager->setTotalRows($this->db_game->getOne("select count(gkey) from pet_goods where $where"));
        $offset = $pager->getOffset();
        $limit = $pager->getLimit();

        $data = $this->db_game->getAll("select * from pet_goods where $where order by createtime desc limit $offset,$limit");
        global $Ggoods;
