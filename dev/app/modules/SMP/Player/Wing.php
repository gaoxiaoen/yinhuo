...
class SMP_Player_Wing extends AdminController
{
    ...
    private function update(){
        ...
        Log::info('SMP_Player_wing',"修改[{$pkey}][$file=$value]");
        $this->db_game->update('player_wing',array($file=>$value),$where);
    }
    ...
    private function show(){
        ...
        $pager = new Pager();
        $pager->setTotalRows($this->db_game->getOne("select count(*) from player_wing where $where"));
        $offset = $pager->getOffset();
        $limit = $pager->getLimit();
        $data = $this->db_game->getAll("select * from player_wing where $where limit $offset,$limit");
