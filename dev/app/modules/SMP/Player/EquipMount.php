...
class SMP_Player_EquipMount extends AdminController
{
    ...
    private function update(){
        ...
        Log::info('SMP_Player_EquipMount',"修改[{$gkey}][$file=$value]");
        $this->db_game->update('mount_goods',array($file=>$value),$where);
    }
    ...
    private function show(){
       ...
       $pos = Jec::getInt('kw_itempos');
        ...
        $pager = new Pager();
        $pager->setTotalRows($this->db_game->getOne("select count(gkey) from mount_goods where $where"));
        $offset = $pager->getOffset();
        $limit = $pager->getLimit();

        $data = $this->db_game->getAll("select * from mount_goods where $where order by createtime desc limit $offset,$limit");
        global $Ggoods;
