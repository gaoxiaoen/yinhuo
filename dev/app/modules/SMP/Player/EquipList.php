...
class SMP_Player_EquipList extends AdminController
{
    ...
    private function show(){
        ...
        if($params['gkey']){
            $where .= " and equip_id={$params['gkey']}";
        }
        ...
        $pager = new Pager();
        $pager->setTotalRows($this->db_game->getOne("select count(ekey) from goods_equip where $where and lossflag=0"));
        $offset = $pager->getOffset();
        $limit = $pager->getLimit();

        $data = $this->db_game->getAll("select * from goods_equip where $where and lossflag=0 order by create_time desc limit $offset,$limit");
        global $Ggoods;
        ...
        foreach($data as &$val){
            ...
            $val['goodsname'] = $Ggoods[$val['equip_id']];
